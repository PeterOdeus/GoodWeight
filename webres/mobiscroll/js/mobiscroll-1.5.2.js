...
(function ($) {
	...
	function Scroller(elm, dw, settings) {
		...
		this.setSingleValue = function(valueObj){
			this.settings.modelValues[valueObj["labelIndex"]] = valueObj["value"];
			console.log("valueObj: " + valueObj);
			console.log("setSingleValue:\nlabelIndex: " + valueObj["labelIndex"] + "\nvalue: " + valueObj["value"]);
			s.onValuesSet(this.settings.modelValues.slice());
		}
		...
		this.scroll = function(t, val, time, orig, index) {
			...
			t.attr('style', (time ? (prefix + '-transition:all ' + time.toFixed(1) 
					+ 's ease-out;') : '') 
					+ (has3d ? (prefix + '-transform:translate3d(0,' 
					+ (val * h + 20) + 'px,0);') : ('top:' + (val * h + 20) + 'px;')));
		...
		this.scrollToSetValue = function(time){
			var dw = $(this.settings.targetSelector);
			var scrollTime = 0.3;
			if(time){
				scrollTime = time;
			}
...
//				}
				...
				that.scroll($(this),0-x, scrollTime, 0 , i);
			});
		}
		...
		this.setValue = function (input) {
			...
			this.settings.modelValues = this.temp.slice(0);
		...
		this.show = function () {
			...
			if(this.settings.targetSelector){
				dw = $(this.settings.targetSelector);
			}
			$('.dwc', dw).remove();
			var tempWheels;
				for (var i = 0; i < s.wheels.length; i++) {
					var dwc = $('<div class="dwc' + (s.mode == 'clickpick' ? ' dwpm' : '') 
							+ '"><div class="dwwc dwrc"><div class="clear" style="clear:both;"></div></div>').appendTo(dw); //insertBefore($('.dwbc', dw));
					// Create wheels
					for (var label in s.wheels[i]) {
						var to1 = $('.dwwc .clear', dwc);
						var w;
						if(label == 'decimalSeparator'){
							var decimalSeparatorClass;
							var language = this.settings.lang;
							if(language == undefined || language == 'en'){
								decimalSeparatorClass = 'dotSeparator';
							}else{
								decimalSeparatorClass = 'commaSeparator';
							}
							w= $('<div class="decimalSeparatorParent dwwl dwrc">'
									+ '<div class="decimalSeparator ' 
									+ decimalSeparatorClass
									+ '"/></div>').insertBefore(to1);
						}else{
							var dwulId = this.settings.targetSelector.substring(1) + label.trim();
							w= $('<div class="dwwl dwrc">' 
								+ (s.mode == 'clickpick' ? '<div class="dwwb dwwbp">+</div><div class="dwwb dwwbm">&ndash;</div>' : '') 
	//							+ '<div class="dwl">' + label 
	//							+ '</div>'
								+ '<div class="dww dwrc"><ul id="' + dwulId +'"></ul><div class="dwwo"></div></div><div class="dwwol"></div></div>').insertBefore(to1);
							// Create wheel values
//							for(var idx = s.wheels[i][label].length - 1; idx >=0; idx++)
//							{
//								$('<li class="val_' + idx + '">' + s.wheels[i][label][idx] + '</li>').data('val', j).appendTo($('ul', w));
//							}
							for (var j in s.wheels[i][label]) {
								$('<li class="val_' + j + '">' + s.wheels[i][label][j] + '</li>').data('val', j).prependTo($('ul', w));
							}
					  	}
					}
				}
							...
							init: function (options) {
								...
								function calc(t, val, anim, orig) {
									...
									//console.log(inst.values);
 									inst.temp[i] = $('#' + t[0].id + ' li:eq(' + aNumber + ')').data('val');
 									var labelIndex = parseInt(t[0].id.substring(t[0].id.length - 1));
 									var scrollerName = t[0].id.substring(0,t[0].id.indexOf("Label"));
 									console.log("scrollerName: " + scrollerName);
 									inst.values[labelIndex] = $('#' + t[0].id + ' li:eq(' + aNumber + ')').data('val');
 									var newValue = $('#' + t[0].id + ' li:eq(' + aNumber + ')').data('val');
 									var valueObj = {"labelIndex": labelIndex, "value": newValue};
 									globalScrollers[scrollerName].setSingleValue(valueObj);
 									//inst.setSingleValue(valueObj);
...
//									$('#hej').html(inst.formatResult());
                            ...
                            if (diff > 0) {
                                ...
                                var dist = stop - start;
									...
									var now = new Date();
									//console.log("END_EVENT time:" + now.getTime()+ ". Target:" + target);
									clearInterval(plustap);
									clearInterval(minustap);
									plustap = false;
									minustap = false;
                            ...
                            $('li:last', t).prependTo(t);
								...
								return this.each(function () {
									...
									globalScrollers[this.id] = scrollers[this.id];
								});
							},
							enable: function() {
								return this.each(function () {
							...
							scrollToSetValue: function(time){
								return this.each(function (time) {
									scrollers[this.id].scrollToSetValue(time);
								});
							},
							getValue: function() {
								if (scrollers[this[0].id])
